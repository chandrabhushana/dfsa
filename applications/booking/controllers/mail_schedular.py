#Rel-3 R3_9 & R3_14 send automated mail to clients & referral agent 
#In which we need to use cron jobs to daily check for appointments, search user emails and 
#send mail
               ...
               if event_id == registration_event:
                   ...
                   for user in user_details:
                       ...
                       if registrations_person == user_id:
                        ...
                        logging.info(user_type)
                        referral_contact_email= user.referral_contact_email
                        logging.info('referral_contact_email====================')
                        logging.info(referral_contact_email)
                        ...
                        mail.send_mail(sender="obs.mornington@gmail.com",to=auth_email,subject = '%s %s %s (Contact Number: %s) has been registered for event \'%s\' %s - %s, %s'%(user.type,user.first_name,user.last_name,user.mobile,events.name,events.start_time,events.end_time,events.date.strftime('%d-%m-%Y')),body="""You have been registered for the above mentioned event""")
                        #mail.send(to=auth_email, subject='Event Upadates', message="You have an event")'''
                        logging.info('Mail sent====================')
                        if referral_contact_email != '':
                            mail.send_mail(sender="obs.mornington@gmail.com",to=referral_contact_email,subject = '%s %s %s (Contact Number: %s) has been registered for event \'%s\' %s - %s, %s'%(user.type,user.first_name,user.last_name,user.mobile,events.name,events.start_time,events.end_time,events.date.strftime('%d-%m-%Y')),body="""Your Clients has been registered for the above mentioned event""")
