...
#Exec the controller files in order to add the validations and drop-downs 
for c in ['activities', 'event_types', 'events', 'registrations']:
    entity=c
    execfile('applications/booking/controllers/%s.py' % c, globals())
...
def archival():
    context = dict(databases=databases)
    return response.render('archival/archival_process.html', context)
...
def import_csv(table, file):
    logging.debug('In import_csv function')
    try:
        table.import_from_csv_file(file)
    except e:
        logging.debug('Import exception error:****'+str(e))
